...
const ComplaintForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitted(true);
    toast({ title: "உங்கள் கோரிக்கை பெறப்பட்டது. விரைவில் தொடர்பு கொள்கிறோம்" });
  };
      ...
      <section id="complaint" className="py-20 bg-muted">
        <div className="container mx-auto px-4 text-center">
          <div className="max-w-md mx-auto p-10 rounded-xl shadow-card bg-secondary text-secondary-foreground">
            ...
            <h3 className="text-2xl font-black mb-2">பெறப்பட்டது!</h3>
            <p className="font-semibold">உங்கள் கோரிக்கை பெறப்பட்டது. விரைவில் தொடர்பு கொள்கிறோம்</p>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section id="complaint" className="py-20 bg-muted">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-black text-center text-foreground mb-4">உங்கள் பிரச்சினையை தெரிவிக்கவும்</h2>
        ...
        <form onSubmit={handleSubmit} className="max-w-lg mx-auto space-y-4 p-8 rounded-xl shadow-card bg-background">
          <input className={inputClass} placeholder="பெயர் *" value={form.name} onChange={e => update("name", e.target.value)} required maxLength={100} />
          <input className={inputClass} placeholder="கைபேசி எண் *" value={form.phone} onChange={e => update("phone", e.target.value)} required type="tel" maxLength={15} />
          ...
          <select className={inputClass} value={form.issueType} onChange={e => update("issueType", e.target.value)} required>
            <option value="">பிரச்சினை வகை *</option>
            {issueTypes.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <textarea className={inputClass + " min-h-[120px]"} placeholder="விவரம்" value={form.description} onChange={e => update("description", e.target.value)} maxLength={1000} />
